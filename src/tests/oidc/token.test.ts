...
describe("handler", () => {
  ...
  test("returns 400 invalid_grant when code_verifier does not match stored code_challenge", async () => {
    const codeVerifier = "test-verifier-123";
    const wrongCodeChallenge = createHash("sha256")
    ...
    dynamoMock
      ...
      .resolves({ Item: { code_challenge: wrongCodeChallenge } });
